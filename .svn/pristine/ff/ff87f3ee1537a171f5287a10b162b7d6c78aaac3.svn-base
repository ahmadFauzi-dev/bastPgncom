...
function __construct()
	   ...
	   public function index()
	   {
			...
			$cek_data;
			$n			 = 0;
			$kemarin 	 	= $this->uri->segment(4) <> "" ? $this->uri->segment(4) : date("Y-m-d", time() - 86400);
			$kemarinlusa   	=  $this->uri->segment(5) <> "" ?  $this->uri->segment(5) : $kemarin;			
			//$kemarin = date("Y-m-d", time() - 86400);
			//$kemarinlusa = date("Y-m-d", time() - 172800);

			$startTime = strtotime($kemarin);
			$endTime = strtotime($kemarinlusa);
		
			for ( $i = $startTime; $i <= $endTime; $i = $i + 86400 )
			{
				$thedate = date("Y-m-d", $i );
				echo " date loop ".$thedate ;
				$tgl = 	$thedate;					
		...
		for ( $i = $startTime; $i <= $endTime; $i = $i + 86400 )
		{
			 $thisDate = date( 'Y-m-d 00:00:00', $i );
			  ...
			  {	
				...
				if($count > 0)
				{
					$data_input["updperson"] 	=  $createdby ;
					$data_input["upddate"] 		=  dateTonum(date('Y-m-d h:i:s')) ;
					
					$this->b_model->update_data('mapmultisource', $filteras, $data_input);
					
				}else
				{
					$data_input["creperson"] 	=  $createdby ;
					$data_input["credate"] 		=  dateTonum(date('Y-m-d h:i:s')) ;
	...
	public function kelengkapan_data_sla()
	{
		...
		$topath = substr(BASEPATH,0,-7).'document/kelengkapandata';	
		...
		foreach ($entries as $datarow) {
			...
			$objPHPExcel->getActiveSheet()->setCellValue('A'.$row, $n+1)
										  ...
										  ->setCellValue('O'.$row, $datarow->alert);										 
		...
		$conditionalStyles = $objPHPExcel->getActiveSheet()->getStyle('A4')->getConditionalStyles();
		array_push($conditionalStyles, $conditional, $conditional2 );               
		$objPHPExcel->getActiveSheet()->getStyle('A$4:AG$'.$row)->setConditionalStyles($conditionalStyles);		
		
		$objWriter = PHPExcel_IOFactory::createWriter($objPHPExcel, 'Excel2007');
		$file = "".$topath."".$subject."-". $tanggalnya. ".xlsx";
		$objWriter->save($file);
