function apps(name, iconCls) {
    ...
    var formanomaliamr = Ext.create('dashboard.form.searchpencapaianvolume');
				...
				items	: [{
					...
					items	: [			
					{
						...
						items	: [{
							...
							id		: 'tableauviz',
							flex	: 1,
							vizUrl	: "http://192.168.104.167:8000/trusted/"+tiketnumber+"/#/views/Dashboardtrenpemakaian/Dashboard2?startdate="+tanggal_awal+"&endate="+tanggal_akhir+"&:refresh=yes&:toolbar=no&:tabs=no&group_id="+group_id+"&username="+username+""
						}]
						//items: gridkelData						
					}]
				}]
					...
					tabPanel.add({
						...
						items: [
						{
							...
							items	: [{
								...
								//height	: 1000,
								flex	: 1,
								vizUrl	: "http://192.168.104.167:8000/trusted/"+tiketnumber2+"/#/views/Trendpemakaianpelanggan/Dashboard1"
							}]
						}]
					});
