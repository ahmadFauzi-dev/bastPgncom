...
public function index()
	{
		...
		$reffjabatan = $this->session->userdata('reffjabatan');	
		$namajabatan = $this->session->userdata('namajabatan');
	...
	public function deletes_entry()
	{
					...
					$datapost = json_decode($data);
					foreach ($datapost as $items)
					{
						$where = array($param =>  $items->rowid);
						$this->b_model->delete_all($table, $where);						
					}	
	...
	*/
	...
	public function insertdatamanualbulk()
	{
		...
		*/
		...
		$data_update = array("volume"	=> $data_input['volume'] * 1000,
	...
	{
		$data_update = array("mmscfd"	=> $this->input->post('mmscfd'),
		...
		$dataupnull = $this->input->post('mmscfd');
		echo gettype($dataupnull);
		die();			 
		$filter[0]['field'] = "rowid";
		$filter[0]['data'] = array("comparison"	=> "eq",
								   "type"		=> "string",
	...
	{
		// $ceksession = false;
		$ceksession = $this->session->userdata('username') != '' ? true : false;
		$data['success'] = $ceksession;
		extjs_output($ceksession);
	...
	public function cekdatarest()
	{
		...
		$dataas = $this->b_model->get_all_data('tbl_jsonpel', $filter , $limit, $offset, '');
		$count = $this->b_model->count_all_data('tbl_jsonpel', $filter);
		$n=0;
	...
	public function treegrid_data()
	{
		...
		$hasil_query	= $this->b_model->get_all_data($view, $filter , $limit,$offset, '');
		...
		/* foreach ($hasil_query as $row) {			
				...
				$count	= $this->b_model->count_all_data($view, $filteras);
				
				if($count > 0 )
				{
					$data[$n]->leaf = false;
					$data[$n]->expanded = true;
					$data[$n]->children = $this->getchild($view,$filteras);
				}
				else{
					$data[$n]->leaf = true;
					$data[$n]->expanded = true;
	...
	function changepassword()
	{
		$this->load->model('m_admin');	
		$user_id  = $this->session->userdata('user_id');
		$pass  = sha1(sha1(md5($this->input->post('pass'))));
		$pass1 = sha1(sha1(md5($this->input->post('pass1'))));
		$data = array ( 'password'  => $pass1 ); 
		
		
		
		$checkpass  = $this->m_admin->checkpassword($user_id,$pass);
		
		
		if ($checkpass['user_id'] == true){
				$this->m_admin->update_entry($data,$user_id,'rev_user','user_id');
				$success = true;
		}
		else{
				$success = false;
