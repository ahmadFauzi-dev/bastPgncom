Ext.define('siangsa.validator.form_editdraftcreator' ,{
		...
		initComponent: function() {
			...
			//var gridtembusan = Ext.create('siangsa.user.gridtembusanedit');
			var gridattdokumen = Ext.create('siangsa.validator.gridattdokumeneditcreator');
			var gridbiayatambahan = Ext.create('siangsa.validator.grideditbiayatambahancreator');
			...
			var mskaryawan = msclass.getmodel("v_revuser");
			var store_karyawan =  msclass.getstore(mskaryawan,"v_revuser",filter);
			store_karyawan.getProxy().extraParams = {
							view :"v_revuser",
							limit : "All"
						};
			store_karyawan.load();
			...
			stb_vendordir.getProxy().extraParams = {
											view :"tb_vendordir",
											limit : "All",
											"filter[0][field]" : "reffvendor",
											"filter[0][data][type]" : 'string',
											"filter[0][data][comparison]" : "eq",
											"filter[0][data][value]" : Init.reffph2																
									};
			stb_vendordir.load();
			...
			Ext.apply(this, {
					...
					{		
							...
							items		: [
							{
								...
								/* {
									...
									id: 'reffph1edit'+pageId,
									name: 'reffph1',
									store: store_karyawan,
									triggerAction: 'all',
									valueField: 'user_id',
									displayField: 'nama',
									queryMode: 'local',
								...
								},
								{
									...
									listeners: {
										change: function(combo, record, index){
												...
												Ext.getCmp("str_jenissuratedit"+pageId).setValue(desk);
										}
									}
								},
								{
									fieldLabel: "Jenis Surat",
									id: "str_jenissuratedit"+pageId,
									name: "str_jenissurat",
									hidden : true
								},
								{
								...
								},
								{
									...
									listeners: {
										change: function(combo, record, index) {
											...
											Ext.getCmp("str_lokasiedit"+pageId).setValue(nama);
										}
									}
								},
								{
									...
									listeners: {
										select: function(combo, record, index) {											
												...
												var store_nm = Ext.getCmp('reffapprovalph2edit'+pageId).getStore();
								...
								if(this.up('form').getForm().isValid()){
				            		this.up('form').getForm().submit({
										...
										success	:function(form, action){
											//myMask.hide();											
											Ext.Msg.alert('Sukses','Data telah terupdate');
				...
				},
				{
					...
					handler: function(){
							...
							if(!Init.winss_formrevisi)
							{
								var formdetailcreator = Ext.create('siangsa.validator.formrevised');
								Init.winss_formrevisi = Ext.widget('window', {
									closeAction	: 'hide',
									width: 500,
									height	: 350,
									autoScroll	:true,
							...
							Init.winss_formrevisi.show();							
							var id = Ext.getCmp('rowidedit'+pageId).getValue();						
							Ext.getCmp('rowidrv'+pageId).setValue(id);
					}
				},
				{
					text: 'Buat BAST',
					...
					handler		: function(grid, rowIndex, colIndex){						
							...
							if(!Init.winss_formcreatebast)
							{
								var formcreatebast = Ext.create('siangsa.validator.formbastcreator');
								Init.winss_formcreatebast = Ext.widget('window', {
									closeAction	: 'hide',
									width: 400,
									height	: 250,
									autoScroll	:true,
							...
							Init.winss_formcreatebast.show();
							
							var id = Ext.getCmp('rowidedit'+pageId).getValue();						
							Ext.getCmp('rowidcb'+pageId).setValue(id);
